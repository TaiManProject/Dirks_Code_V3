...
//    commonTool.log(QString("using seperator ---> %1").arg(separator));
	...
	if (!file) {
        std::string error_message = "No valid input file was given, please check the given filename."+filename;
...
void FaceMatchingManager::train(QString& faceRecognitionDir, std::vector<cv::Mat>& additionalImages, QMutex& openCVMutex) {
    if (enableMultiLevel) {
        trainMulti(faceRecognitionDir, additionalImages, openCVMutex);
    } else {
...
void FaceMatchingManager::trainMulti(QString& faceRecognitionDir, std::vector<cv::Mat>& additionalImages, QMutex& openCVMutex) {
    ...
    controllerMulti = new FaceMatchingMuti(images, labels, openCVMutex);
}
