						   ...
						   )
{
	...
	if (level == _INTEGRALIMAGES_LOWER_){
		searchWinSize = param.SearchWidthLower;
	} else if (level == _INTEGRALIMAGES_UPPER_){
		searchWinSize = param.SearchWidthUpper;
	}
...
int Synthesis::FindKNN(IntegralImages* sourceImages, IntegralImages* targetImages, IntegralImage testImage)
{
    ...
    /*****************  this function is used to the k nearest neighbor for each patch  *****************/
...
*/
    // compute the number of patches for each row and column
