...
void FaceSynthesisManager::synthesize(cv::Mat inputImg) {
    ...
    timeNeedForFindKNN = t.restart();

    commonTool.log(QString("Time needed for finding KNN: %1 ms.").arg(timeNeedForFindKNN));
    ...
    //tempOutputImage = new QImage((unsigned char*)Sketch->FinalResult.data, // uchar* data
    //        Sketch->FinalResult.cols, Sketch->FinalResult.rows, // width height
    //        QImage::Format_Indexed8); //format

    //outputImage = tempOutputImage;

    //copy the candidate patch
    ...
    for (int i = 0; i < totalNumberOfPatch; i++){
        //cout << "totalNumberOfPatch " << i << "/" << totalNumberOfPatch << endl;
        ...
        for (int j = 0; j < Sketch->NumOfCan; j++){
            ...
            //candidatePatch->sourcePatchData = Sketch->ImagePatchLower.at(i).PatchData.at(j).PhotoPatchData.clone();
            //candidatePatch->targetPatchData = Sketch->ImagePatchLower.at(i).PatchData.at(j).SketchPatchData.clone();
            ...
            //candidatePatch->sourcePatchData.convertTo(candidatePatch->sourcePatchData, CV_8UC1);
            //candidatePatch->sourcePatchData.convertTo(candidatePatch->sourcePatchData, CV_8UC1);

            //cout << "ResultWeight " << i*Sketch->NumOfCan + j << "/" << Sketch->getResultWeight().rows * Sketch->getResultWeight().cols<< endl;
    ...
    timeNeedForMinEng = t.elapsed();

    commonTool.log(QString("Time needed for finding min energy: %1 ms.").arg(timeNeedForMinEng));
...
void FaceSynthesisManager::clean()
{
	...
	//delete inputImage;
	//delete outputImage;
	//delete faceRecognitionImage;

	//inputImage = new QImage();
	//outputImage = new QImage();
	//faceRecognitionImage = new QImage();
}
...
int FaceSynthesisManager::getTimeNeedForFindKNNInMS(){
	return timeNeedForFindKNN;
}
int FaceSynthesisManager::getTimeNeedForMinEngInMS(){
	return timeNeedForMinEng;
}
